...
import { X, Terminal, Link, AlignLeft, Calendar, Save, Trash2, Edit3, ShieldAlert, RefreshCcw, MessageSquare } from 'lucide-react';
...
export default function TaskDetailsModal({ task, isOpen, onClose, onUpdate, onDelete }) {
    ...
    const handleSave = () => {
        onUpdate(task.id, { title, description, link, type, duration, resumePoint, completionNote });
        setIsEditing(false);
    };
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-2xl bg-[#161b22] border border-[#30363d] rounded-2xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
                {/* Header */}
                    ...
                    <div className="border-t border-[#30363d] pt-8">
                        ...
                        {isEditing ? (
                            <textarea
                                ...
                                className="w-full bg-[#0d1117] border border-neon-green/20 p-4 rounded-xl text-sm text-gray-200 font-sans outline-none focus:border-neon-green/40 transition-all resize-none"
                            />
                        ) : (
                ...
                <div className="p-5 bg-[#0d1117]/50 border-t border-[#30363d] flex justify-between items-center backdrop-blur-md">
                    <button
                        onClick={() => { if (window.confirm('¿ELIMINAR TAREA permanentemente?')) onDelete(task.id); }}
