import { Terminal, ShieldCheck, Check, Trash2, ExternalLink, Filter, Search, ChevronRight, RefreshCcw } from 'lucide-react';

export default function TaskList({ tasks, onToggleTask, onDeleteTask, filter, setFilter, searchTerm, setSearchTerm, onOpenDetails }) {
    const filterOptions = [
            ...
            <div className="p-4 border-b border-[#30363d] flex justify-between items-center bg-[#21262d]/50 backdrop-blur-md">
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-xs font-mono font-bold text-white tracking-widest uppercase">
                        Registro de Tareas <span className="text-neon-blue opacity-50">v3.6</span>
                    </h2>
                </div>
                ...
                {tasks.length === 0 ? (
                    ...
                    tasks.map(task => {
                        ...
                        return (
                            <div
                                key={task.id}
                                className={`group flex gap-3 pb-3 border border-transparent last:border-0 hover:bg-[#161b22] px-3 py-4 rounded-xl transition-all relative cursor-pointer ${task.isCompleted ? 'task-glitch-active' : ''} ${task.type === 'EXAMEN' && !task.isCompleted ? 'critical-threat-pulse border-red-500/30' : 'border-b-[#30363d]/30'}`}
                            ...
                            >
                                ...
                                <div className="min-w-0 flex-1">
                                    <div className="flex items-center gap-2 mb-1.5">
                                        ...
                                        <div className="flex flex-col items-end ml-auto text-[10px] font-mono leading-tight">
                                            ...
                                            <span className={`font-bold ${task.isCompleted ? 'text-neon-green/80' : 'text-gray-700'}`}>
                                                {task.isCompleted && task.completedAt
                                                    ? new Date(task.completedAt).toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit' })
                                                    : task.date ? new Date(task.date).toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit' }) : '--/--'}
                                            </span>
                                        </div>
                                    </div>
                                    <div className="flex items-start gap-2">
                                        <p className={`text-[15px] text-gray-200 leading-tight break-words group-hover:text-neon-blue transition-colors ${task.isCompleted ? 'font-bold text-neon-green/90' : 'font-medium'}`}>
                                            {task.title}
                                        </p>
                                    </div>
                                ...
                                <button
                                    ...
                                    className="opacity-0 group-hover:opacity-100 p-2 text-gray-700 hover:text-red-500 hover:bg-red-500/10 rounded-lg transition-all self-center relative z-10"
                                >
